<template>
    <div>
        ...
        <div class="mb-8 flex items-center">
            ...
            <router-link class="flex items-center mr-4 p-2 bg-green-500 hover:bg-green-900 rounded" :to="{ name: 'UsersCreate' }">
                        ...
                        <table class="table-auto w-full text-center text-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border px-2 py-2">{{ $t('Usename') }}</th>
                                    <th class="border px-2 py-2">{{ $t('Name') }}</th>
                                    <th class="border px-2 py-2">{{ $t('Email') }}</th>
                                    <th class="border px-2 py-2">{{ $t('Gender') }}</th>
                                    <th class="border px-2 py-2">{{ $t('Date') }}</th>
                                    <th class="border px-2 py-2">{{ $t('Role') }}</th>
                            ...
                            <tbody>
                                <tr v-for="(user, index) in users" :key="index">
                                    ...
                                    <td class="border px-2 py-2">{{ user.gender }}</td>
                                    <td class="border px-2 py-2">{{ user.date }}</td>
                                    <td class="border px-2 py-2">{{ user.isAdmin === 1 ? 'Admin' : 'User' }}</td>
                                    <td class="border px-2 py-2">
                                        <div class="flex justify-center items-center">
                                            <div class="inline-block mx-1 p-1 rounded bg-green-500 hover:bg-green-800">
                                                <router-link class="flex items-center" :to="{ name: 'UsersEdit', params: { id: user.id } }">
                                                    <span class="inline-block">
